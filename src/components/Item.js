...
const Item = ({post}) => {
  ...
  // console.log(post)
  const [likePost, setLikePost] = useState(getDatafromLS());
  ...
  // setLikePost(...likePost, like)
  // localStorage.setItem('likes', JSON.stringify(likePost));
  return ( 
    <div>
        <div key={post.id} className="empty">
          <div className="show__header">
            ...
            <div className="show__header-name">
              {post.author}<div className="date">{post.createdAt}</div>
            </div>
          </div>
          ...
          <div className="show__reactions">
            <span className="reactions">
              <FaRegThumbsUp onClick={likeHandler} /> <span className="reactions-text">{like} Likes</span>
            </span> 
